 ...
 * 错误提示文案（已移除国际化，直接中文）
 */
function getSubTitle(isChunkError: boolean, isOffline: boolean): string {
  if (!isChunkError) return '抱歉，页面遇到了一些问题，请刷新页面或返回首页。';
  return isOffline
    ? '网络连接已断开，请检查网络后重新加载。'
    : '页面资源加载失败，请重新加载重试。';
}
  ...
  return (
    <Card variant="borderless" style={{ margin: 24 }}>
      <Result
        ...
        subTitle={getSubTitle(isChunkError, isOffline)}
        extra={[
          isChunkError && (
